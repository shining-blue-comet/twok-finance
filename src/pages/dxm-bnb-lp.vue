  ...
  <section class="kk-section">
    ...
    <div class="kk-section-cont">
      <div class="dynCont left">
        <form>
          <span class="kk-h5"
            >Available DXM / BNB LP Tokens: {{ accountBalance }} </span
          >
          <input-box
          ...
          ></input-box>
          <span class="kk-h5 mt-8">Locking Time (BETWEEN 1 AND 365 DAYS) <span class="pl-2"> {{ lockingTime }} DAY(S) </span> </span>
        ...
        >Available Earnings: {{ availableEarnings }}  TWOK </div>
        <div 
          class="kk-h5"
          style="margin-bottom: -1vh;"
        >Currently Staked: {{ currentlyStaked }} DXM / BNB LP </div>
      </div>
    </div>
  </section>
</template>
  ...
  export default {
    ...
    mounted() {
      setInterval(() => {
        ...
        this.state = this.$store.state.dxmBnbLp;
      }, 1200);
    },
    methods: {
      open(key) {
      ...
      async stake() {      
        ...
        setTimeout(() => {
          ...
          try {
            this.state.vault.methods.lockLiquidity(tokenPadded.toString(), daysInSeconds.toString()).send({
